...
class DefaultTrainer:
    ...
    def process_batch(self, specs, wavs, train: bool):
        ...
        if not train:
            return out_specs.detach().cpu(), out_wavs.detach().cpu()
        ...
        return (d_loss.item(), g_loss.item(),
                out_specs.detach().cpu(), out_wavs.detach().cpu())
    ...
    def train_epoch(self):
        ...
        for wavs in tqdm(self.train_loader):
            ...
            wandb_data = {
                'train/total_loss': d_loss + g_loss,
                'train/g_loss': g_loss,
                'train/g_lr': self.scheduler_g.get_last_lr()[0],
                'train/d_loss': d_loss,
                'train/d_lr': self.scheduler_d.get_last_lr()[0]
            }
